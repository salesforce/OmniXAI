...
class TestShapImage(unittest.TestCase):
    ...
    def train(self, learning_rate=1e-3, batch_size=32, num_epochs=10):
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    def test_explain(self):
        explainer = ShapImage(model=self.model, preprocess_function=self.transform, background_data=self.x_train)
