...
class TestCE(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.img = Image(PilImage.open(os.path.join(directory, "images/camera.jpg")).convert("RGB"))
    ...
    def test_explain(self):
        explainer = CounterfactualExplainer(
            model=self.model, preprocess_function=self.preprocess, binary_search_steps=2, num_iterations=100
        )
